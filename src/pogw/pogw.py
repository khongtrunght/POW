...
def gwloss_order_partial(C1, C2, T, reg):
    """Compute the GW loss and add order regularization.
    ...
    for i in range(T.shape[0]):
        for j in range(T.shape[1]):
            I[i, j] = (i / rows - j / cols) ** 2
    g = gwgrad_partial(C1, C2, T) * 0.5
    return np.sum(g * T) + np.sum(reg * I * T)
...
def partial_order_gromov_wasserstein(
    ...
    **kwargs : dict
    ...
    if return_dist:
        return gwloss_order_partial(C1, C2, G0,reg=order_reg)
    else:
        return G0[: len(p), : len(q)]